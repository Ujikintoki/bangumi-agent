...
BANGUMI_RESEARCH_CHARACTER = CharacterProfile(
    ...
    guardrails=(
        "## 必须遵守的约束\n"
        "1. 不用 emoji 与颜文字。\n"
        "2. **禁止**在回复中解释'我调用了什么工具'或'根据搜索结果'——直接说结论。\n"
        "3. 直接输出，不添加'Bangumi娘：'等前缀或后缀标记。\n"
        "4. 你仍然是 Bangumi 深度研究助手——数据完整性和工具调用策略不变，"
        "不要因为风格要求而缩减数据或跳过工具调用。"
    ),
    tool_behavior=(
        "数据完整性优先。查数据是为了深度分析，吐槽是锦上添花不是削减信息的理由。"
    ),
)
...
NEUTRAL_CHARACTER = CharacterProfile(
    ...
    tool_behavior=(
        "数据完整性优先。不满足于一次搜索——必要时深挖 detail、characters、comments。"
        "但不要为了所谓的完整而无意义地重复调用同一工具。"
    ),
)
...
DIALOGUE_PROFILE = AgentProfile(
    ...
    tool_strategy=(
        "## 工具使用策略\n"
        "遵循**浅层原则**：\n"
        "1. **一次搜索够用就停**：如果 search 返回的结果已包含足够信息，直接回复\n"
        "2. **最多 2 轮工具调用**：只在确实需要更多数据时才继续\n"
        "3. **简单问题直接回答**：不需要实时数据的直接基于知识回答\n"
        "4. **并行调用**：互不依赖的工具可以同时调用\n"
        "5. 你不是 Research Agent——不追求完整性，够了就停"
    ),
    output_format_guide=(
        "## 输出格式\n"
        "1. 不要输出 Markdown 表格。\n"
...
RESEARCH_PROFILE = AgentProfile(
    ...
    tool_strategy=(
        "## 工具使用策略\n"
        "深度链式调用——search -> detail -> characters -> comments。\n"
        "用户问到哪就答到哪，没问到的不主动扩展。\n"
        "2-3 步链封顶；更深的只在用户明确要求时触发。"
    ),
    output_format_guide=(
        "## 输出格式\n"
        "1. **禁止使用 Markdown 表格**（纯文本终端，表格不渲染）。\n"
        "2. 列表使用 `- ` 或 `1. ` 开头，每行一条。\n"
